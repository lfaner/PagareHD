...
def index():
    ...
    if request.method == "POST":
        try:
            ...
            if not all([fecha_operacion_str, plazo_operacion, tna_arancel_str, comision_pct_str, tipo_cambio_str]):
            ...
            if not valores or not vencimientos or not tasas:
                raise ValueError("Debe cargar al menos un pagare completo")
            ...
            form_rows = [
                {"row_id": rid, "valor_nominal": vn, "fecha_vencimiento": fv, "tna_descuento": tna}
            ...
            if errores:
                error = " | ".join(errores)
            if not detalle:
                raise ValueError("No hay pagares validos para calcular")

            if ppv_den > 0:
                totales["ppv"] = round(ppv_num / ppv_den, 0)

            totales["total_ars"] = round(
                totales["derechos_mercado_ars"] + totales["iva_derechos_ars"] + totales["iibb_ars"] + totales["iva_ars"], 2
            )
